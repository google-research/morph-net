# TensorFlow-Slim image classification model library with Morph-Net integration

## Instructions to train with Morph-Net

Morph-Net parameters are in `train_image_classifier.py` from lines 546-552.

Change models as described in `nets/net_factory.py` lines 30-37.

Structure should be exported to `TRAIN_DIR/ee`.

num_clones specifies number of GPUs for multi-GPU training.

```shell
DATASET_DIR=/path/to/imagenet/tf-records
TRAIN_DIR=/path/to/morph-net_logs
